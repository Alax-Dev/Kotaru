...
class WebViewExecutor @Inject constructor(
	...
	suspend fun tryResolveCaptcha(exception: CloudFlareException, timeout: Long): Boolean = mutex.withLock {
			...
			val result = runCatchingCancellable {
				withContext(Dispatchers.Main.immediate) {
					...
					try {
						...
						val effectiveTimeout = if (attempt > 0) timeout * 2 else timeout
						withTimeout(effectiveTimeout) {
							suspendCancellableCoroutine { cont ->
								webView.webViewClient = AutoCaptchaContinuationClient(
									cookieJar = cookieJar,
		...
		lastException?.let { exception.addSuppressed(it) }
		false
	}
